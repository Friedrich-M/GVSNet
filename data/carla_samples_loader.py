...
class LoadSamples(Dataset):
    def __init__(self, opts):
        ...
        files = sorted([f.__str__() for f in pathlib.Path(self.opts.data_path).rglob('*.png')]) # get all png files
        sem_files = [f for f in files if f.endswith('semantics.png')] # get all semantic files
        assert len(
    ...
    def __getitem__(self, index):
        ...
        data_dict['k_matrix'] = self.get_k_matrix(height=self.opts.height, width=self.opts.width) # K matrix
        data_dict = {k:v.float() for k,v in data_dict.items()} # convert to float
        return data_dict
    ...
    def read_semantics(self, filename):
        ...
        sem_img = F.interpolate(input=sem_img.float().unsqueeze(0).unsqueeze(
            0), size=(self.opts.height, self.opts.width), mode='nearest').squeeze(0) # resize to 256x256
    ...
    def read_color(self, filename):
        ...
        img_np = cv.resize(img, (self.opts.width, self.opts.height)) / 255.0 # resize to 256x256 and normalize to [0,1]
        img_np = (2*img_np)-1.0 # normalize to [-1,1]
