...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_ch, k):
        ...
        self.conv_1 = nn.Conv2d(
            in_ch, in_ch//2, kernel_size=k, stride=1, padding=k // 2)
        self.conv_2 = nn.Conv2d(
            in_ch // 2, in_ch, kernel_size=k, stride=1, padding=k // 2)
...
class ConvBlock(nn.Module):
    def __init__(self, inp_chans, out_chans, k_size, down_sample=True, use_no_relu=False):
        ...
        self.conv_0 = nn.Conv2d(
            inp_chans, out_chans, kernel_size=k_size, stride=stride_0, padding=k_size // 2) 
            # if stride_0 == 0: 
            # H_out = (H_in + 2*padding - kernel_size)/stride + 1 = (256 + 2*1 - 3)/1 + 1 = 256
            # W_out = (W_in + 2*padding - kernel_size)/stride + 1 = (256 + 2*1 - 3)/1 + 1 = 256
            # else: 如果进行下采样，那么 H_out = W_out = 128
            # H_out = (H_in + 2*padding - kernel_size)/stride + 1 = (256 + 2*1 - 3)/2 + 1 = 128
            # W_out = (W_in + 2*padding - kernel_size)/stride + 1 = (256 + 2*1 - 3)/2 + 1 = 128
        self.conv_1 = nn.Conv2d(
            out_chans, out_chans, kernel_size=k_size, stride=1, padding=k_size // 2)
        nn.init.xavier_normal_(self.conv_0.weight.data, gain=1.0) # 让权重服从均值为0，方差为1的正态分布
        nn.init.xavier_normal_(self.conv_1.weight.data, gain=1.0) # 让权重服从均值为0，方差为1的正态分布
        self.non_linearity_0 = F.relu # 激活函数， relu
        self.non_linearity_1 = F.relu # 激活函数， relu
        if use_no_relu: # 如果no_relu，那么就不使用激活函数
            self.non_linearity_1 = nn.Sequential()
        else:
            self.non_linearity_1 = F.relu

    def forward(self, x):
        x1 = self.non_linearity_0(self.conv_0(x)) # 第一层卷积，使用relu激活函数
        x2 = self.non_linearity_1(self.conv_1(x1)) # 第二层卷积，使用relu激活函数
        return x2
...
class DeconvBlock(nn.Module):
    def __init__(self, inp_chans, out_chans):
        ...
        self.conv_0 = nn.Conv2d(inp_chans, out_chans,
...
class BaseEncoderDecoder(torch.nn.Module):
    ...
    def __init__(self, in_chans, feat_size=32, output_feats=96, kernel_size=3):
        ...
        self.conv_block_3 = ConvBlock(
            int(2 * feat_size), int(4 * feat_size), kernel_size)  # H/8
        self.conv_block_4 = ConvBlock(
            int(4 * feat_size), int(8 * feat_size), kernel_size)  # H/16
        self.conv_block_5 = ConvBlock(
            int(8 * feat_size), int(16 * feat_size), kernel_size)  # H/32
        self.conv_block_6 = ConvBlock(
            int(16 * feat_size), int(16 * feat_size), kernel_size)  # H/64
        self.conv_block_7 = ConvBlock(
            int(16 * feat_size), int(16 * feat_size), kernel_size)  # H/128
        self.up_conv_7 = DeconvBlock(
            int(16 * feat_size), int(16 * feat_size))          # H/64
        self.up_conv_6 = DeconvBlock(
            int(32 * feat_size), int(16 * feat_size))          # H/8
        self.up_conv_5 = DeconvBlock(
            int(32 * feat_size), int(16 * feat_size))          # H/8
        self.up_conv_4 = DeconvBlock(
            int(24 * feat_size), int(12 * feat_size))          # H/8
        self.up_conv_3 = DeconvBlock(
            int(16 * feat_size), int(8 * feat_size))           # H/8
        self.up_conv_2 = DeconvBlock(
            int(10 * feat_size), int(3 * feat_size))           # H/8
        self.up_conv_1 = DeconvBlock(
            int(4 * feat_size), int(3 * feat_size))            # H/4
...
class ConvNetwork(torch.nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.blending_alpha_seg_pred = nn.Sequential(ResBlock(enc_features, 3),
                                                     ...
                                                     nn.SyncBatchNorm(
                                                         enc_features),
                                                     ConvBlock(enc_features, int(
                                                         num_planes*(self.out_seg_chans+1))//2, 3, down_sample=False),
                                                     nn.SyncBatchNorm(
                                                         int(num_planes*(self.out_seg_chans+1))//2),
                                                     ConvBlock(int(num_planes*(self.out_seg_chans+1))//2,
                                                               int(num_planes *
                                                                   (self.out_seg_chans+1)), 3,
                                                               down_sample=False,
                                                               use_no_relu=True))
    ...
    def forward(self, input_img):
        ...
        alpha_and_seg = alpha_and_seg.view(
            b, self.num_planes, self.out_seg_chans+1, h, w)
